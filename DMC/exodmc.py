...
class exodmc(object):
    ...
    def __init__(self, star_ID, star_dist):
        ...
        if isinstance(star_dist,(list)) is not True: star_ID=[star_dist]
    ...
    def DImode(self, xlim, ylim, lxunit='as', lyunit='Mjup', verbose=True, plot=True, savefig=True):
        ...
        for ll in range(ns):
            ...
            start = time.time()
            det=np.zeros((self.x_nsteps, self.y_nsteps, self.norb), dtype=np.float32) #AC
            ...
            rho_mins = np.min(self.rho[ll], axis=1)
            rho_maxs = np.max(self.rho[ll], axis=1)
            ff_new = (rho_mins < min_sma) & (rho_maxs > max_sma)
            
            mm = np.where(self.M2 > max_mass)[0]
            
            is_mass_in_range = (self.M2 > min_mass) & (self.M2 < max_mass)
            index = rlim[:, None, :] < self.M2[None, is_mass_in_range, None]

            index2 = np.zeros_like(det, dtype=bool)
            index2[:, is_mass_in_range, :] = index
            ...
            map = np.where(map == 1/self.norb, 0, map)
